...
def filter_command(cmd: list[str]):
    """Filter command list to be suitable for subprocess on Windows.
    Remove whitespaces like ;.-:@#$%^&*()<>?/\|`~{}[]'" from each argument.
    Args:
        cmd (list[str]): Command list.
    Returns:
        list[str]: Filtered command list.
    """
    filtered = str.maketrans("", "", " ;.-:@#$%^&*()<>?/\|`~{}[]'\"")
    program_name = cmd[0]
    assert "Scaffolder.exe" in program_name, "This function is only for Scaffolder.exe"
    cmd = [program_name] + [arg.translate(filtered) for arg in cmd[1:]]
    return cmd


def run_scaffolder(
    input_stl, pattern, grid, coff, thickness=None, cwd="", options=[], config={}
):
    ...
    if config.get("output_stl"):
        arg = [
            os.path.join("..", "..", "Scaffolder.exe"),
            str(input_stl),
            str(config["output_stl"]),
            "-n",
            str(pattern),
            "-c",
            str(coff),
    ...
    try:
        process = subprocess.Popen(filter_command(arg), cwd=cwd, shell=True)
...
def run_batch(config, result: str = "time_memory.txt", result_dir: str = ""):
    ...
    # If an attacker can control the input leading into the open function,
    # they might be able to read sensitive files and launch further attacks with that information.
            ...
            for grid in grid_size:
                for _coff in coff:
                    for t in thickness:
                        try:
                            print(
                                f"{i}/{total} name={p} grid={grid} coff={_coff} thickness={t}: ",
                                end="",
                            )
                            out.write(f"{p},{grid},{_coff * math.pi:.6},{t},")

                            elapsed, mem, filesize = run_scaffolder(
                                input_stl,
                                p,
                                grid,
                                _coff,
...
def run_list(config, result: str = "time_memory_.txt", result_dir=""):
    input_stl = config["input_stl"]
    params = config["list"]
    with open(os.path.join(result_dir, result), "w") as out:
        total = len(params)
        ...
        for param in params:
            if type(param) is not dict:
                ...
                continue
            if (
                not param.get("pattern")
                or not param.get("grid_size")
                or not param.get("coff")
            ):
                print('Invalid {param}: Missing "pattern", "grid_size", "coff"')
                continue
            thickness = 0 if not param.get("thickness") else param["thickness"]
            p = param.get("pattern")
            grid = int(param.get("grid_size", 0))
            coff = float(param.get("coff", 0))
            try:
                # tracking infomation
                print(
                    f"{i}/{total} name={p} grid={grid} coff={coff} thickness={thickness}: ",
                    end="",
                )
    ...
    if len(sys.argv) == 2:
        ...
        if not os.path.exists(result_dir):
            ...
            if _old_files_size > 0:
                print(
                    f"There are {_old_files_size} result files in directory {result_dir}"
                )
        ...
        if not os.path.exists(test_file):
            print(
                f'Not found {test_file}: "input_stl" parameter is relative to the folder {result_dir}'
            )
            sys.exit(0)
        # default options
